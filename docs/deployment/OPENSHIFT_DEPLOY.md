...
- A **Deployment** and **Service** (`maas-finops`) serving the plugin's static assets (including `remoteEntry.js`) via Nginx on port 8080
- A **BFF Deployment** and **Service** (`maas-finops-bff`) running the plugin's backend service on port 3000 (enabled by default)
...
The `proxyService` entry tells the dashboard to forward requests from `/maas-finops/api/*` to the BFF service, rewriting the path to `/api/*` and forwarding the user's Bearer token (`authorize: true`).
...
```bash
oc get pods -n cp-maas-finops
```

You should see pods for `maas-finops` (and `maas-finops-bff` if BFF is enabled), all in `Running` status.
...
Retrieve the current config, remove the `maasFinops` entry, and re-apply:
...
import json, sys
config = json.load(sys.stdin)
config = [e for e in config if e.get('name') != 'maasFinops']
print(json.dumps(config))
...
```bash
helm uninstall maas-finops -n cp-maas-finops
oc delete namespace cp-maas-finops   # optional: remove the namespace entirely
```
