...
|---|---|---|---|
| `package.json` | `name` | `maas-finops` | `{your-plugin}` |
| `package.json` | `module-federation.name` | `maasFinops` | `{yourPlugin}` (camelCase) |
| `package.json` | `module-federation.proxy[].path` | `/maas-finops` | `/{your-plugin}` |
| `package.json` | `module-federation.proxy[].pathRewrite` | `/maas-finops` | `/{your-plugin}` |
| `package.json` | `module-federation.local.port` | `9500` | Any unused port (see [Port allocation](#port-allocation)) |
| `plugin.yaml` | `name` | `maas-finops` | `{your-plugin}` |
| `plugin.yaml` | `displayName` | `MaaS FinOps` | Your plugin name |
| `plugin.yaml` | `image.repository` | `quay.io/.../maas-finops` | Your image repository |
| `plugin.yaml` | `install.helm.registry` | `oci://quay.io/.../maas-finops` | Your OCI chart registry |
| `plugin.yaml` | `remote.spec.name` | `maasFinops` | `{yourPlugin}` (camelCase) |
| `plugin.yaml` | `remote.spec.scope` | `maasFinops` | `{yourPlugin}` (must match `name`) |
| `plugin.yaml` | `remote.spec.remoteEntry` | `.../maas-finops/...` | Your deployed image URL |
| `plugin.yaml` | `remote.spec.paths[0].path` | `/maas-finops` | `/{your-plugin}` |
| `plugin.yaml` | `remote.spec.paths[0].extensions` | `maasFinops/extensions` | `{yourPlugin}/extensions` |
| `plugin.yaml` | `remote.spec.paths[1].path` | `maasFinops/Icon` | `{yourPlugin}/Icon` |
| `src/rhoai/extensions.ts` | area `id` | `maas-finops` | `{your-plugin}` |
| `src/rhoai/extensions.ts` | plugin section `id` | `maas-finops` | `{your-plugin}` |
| `src/rhoai/extensions.ts` | plugin section `title` | `MaaS FinOps` | Your plugin name |
| `src/rhoai/extensions.ts` | plugin section `group` | `1_maas_finops` | `{N}_{your_plugin}` |
| `src/rhoai/extensions.ts` | nav item `id`s | `maas-finops-*` | `{your-plugin}-{page}` |
| `src/rhoai/extensions.ts` | nav item `href`/`path` | `/maas-finops/*` | `/{your-plugin}/*` |
| `src/rhoai/extensions.ts` | route `path` | `/maas-finops/*` | `/{your-plugin}/*` |
| `src/bootstrap.tsx` | `Router basename` | `/maas-finops` | `/{your-plugin}` |
| `config/webpack.common.js` | MF plugin `name` | `maasFinops` | `{yourPlugin}` (camelCase) |
| `config/moduleFederation.js` | `name` | `maasFinops` | `{yourPlugin}` (camelCase) |
| `config/webpack.dev.js` | proxy `context` | `/maas-finops` | `/{your-plugin}` |
| `config/webpack.dev.js` | `port` | `9500` | Same as `package.json` port |
| `.env.development` | `URL_PREFIX` | `/maas-finops` | `/{your-plugin}` |
| `chart/Chart.yaml` | `name` | `maas-finops-chart` | `{your-plugin}-chart` |
| `chart/values.yaml` | `image.repository` | `quay.io/.../maas-finops` | Your image repository |
| `chart/values.yaml` | `ingress.path` | `/maas-finops` | `/{your-plugin}` |
