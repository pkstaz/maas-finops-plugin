...
The BFF is a Go service that runs alongside the plugin's webpack dev server.
...
You should see `listening on :3000`. Mock mode (`MOCK_DATA=true`) does not need a cluster. For live data, run `cd bff && DEV_MODE=true PORT=3000 go run ./cmd/backend` with a valid kubeconfig.
>
> **Note:** The `proxyService` entry in the `MODULE_FEDERATION_CONFIG` (Step 2) is what tells the dashboard to forward `/maas-finops/api/*` requests to the BFF at `localhost:3000`. If you omit the `proxyService` block, those requests will hit the dashboard's SPA fallback and return HTML instead of JSON.
...
Open the dashboard URL in your browser. You should see the RHOAI Dashboard with your plugin loaded in the sidebar, including the Namespace Summary page under the MaaS FinOps section.

### How it works

- The backend reads `MODULE_FEDERATION_CONFIG`, registers a proxy `/_mf/maasFinops/*` that routes to `http://localhost:9500/*`.
...
```bash
MODULE_FEDERATION_CONFIG=[{"name":"maasFinops","backend":{"remoteEntry":"/remoteEntry.js","tls":false,"localService":{"host":"localhost","port":9500},"service":{"name":"placeholder","namespace":"opendatahub","port":8080}},"proxyService":[{"path":"/maas-finops/api","pathRewrite":"/api","authorize":true,"tls":false,"localService":{"host":"localhost","port":3000},"service":{"name":"placeholder","namespace":"opendatahub","port":3000}}]}]
```
...
The BFF is a Go service. In a **separate terminal**, from the plugin project root:

```bash
make dev-bff
```

You should see `listening on :3000`. For live cluster data instead of mocks: `cd bff && DEV_MODE=true PORT=3000 go run ./cmd/backend`.
>
> **Note:** The `proxyService` entry in `env.local` (Step 6) tells the dashboard to forward `/maas-finops/api/*` requests to the BFF. Without it, those requests return HTML instead of JSON.
...
| Field | Description |
|---|---|
| `proxyService[].path` | URL path prefix the dashboard intercepts (e.g. `/maas-finops/api`). |
...
2. Start the BFF service with `make dev-bff`
...
The frontend receives HTML instead of JSON. This means the request to `/maas-finops/api/*` is not being proxied to the BFF and is hitting the SPA fallback instead.
...
- **BFF not running**: Ensure the BFF is running (`make dev-bff`). You should see `listening on :3000`.
- **No kubeconfig**: Live mode needs `oc login` (or a kubeconfig). Use `make dev-bff` for mock data.
