...
FRONTEND_IMAGE ?= maas-finops
BFF_IMAGE      ?= maas-finops-bff
CHART_NAME     ?= maas-finops-chart
VERSION        ?=
BUILDER        ?= podman
IMAGE_TAG      ?= latest
...
.PHONY: typecheck typecheck-frontend

typecheck: typecheck-frontend ## TypeScript type checking (frontend)
...
validate-bff: lint-bff test-bff
...
build-bff:
	cd bff && go build -o bin/backend ./cmd/backend
...
dev-bff: ## Start BFF with mock data on port 3000
	cd bff && DEV_MODE=true MOCK_DATA=true PORT=3000 go run ./cmd/backend
